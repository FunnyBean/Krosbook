...
var OperatorGuard = (function () {
    function OperatorGuard(userService, router) {
        this.userService = userService;
        this.router = router;
        this.loadAuth();
    }
    OperatorGuard.prototype.canActivate = function (next, state) {
        var result = this.loadAuth();
        return result;
    };
    OperatorGuard.prototype.loadAuth = function () {
        var _this = this;
        return Observable_1.Observable.create(function (observer) {
            _this.userService.myProfile().map(function (res) { return res.json().roles; }).subscribe(function (result) {
                ...
                if (hasAdminRole)
                    ...
                    observer.next(false);
                    _this.router.navigate(['/home/reservations/cars']);
                }
                observer.complete();
            }, function (error) {
                observer.next(false);
