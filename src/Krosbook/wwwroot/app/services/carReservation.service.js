...
var CarOrderService = (function () {
    ...
    CarOrderService.prototype.addOrder = function (carId, dateStart, dateEnd, destination, GPSSystem, privateUse, requirements, travelInsurance, reservationState) {
        var headers = new http_1.Headers();
        headers.append('Content-Type', 'application/json');
        return this.http.post('http://funnybean.cloudapp.net/api/reservations/cars', JSON.stringify({ carId: carId, dateStart: dateStart, dateEnd: dateEnd, destination: destination, GPSSystem: GPSSystem, privateUse: privateUse, requirements: requirements, travelInsurance: travelInsurance, reservationState: reservationState }), { headers: headers });
    };
    ...
    CarOrderService.prototype.getReservations = function (id, from, to) {
        var headers = new http_1.Headers();
        headers.append('Content-Type', 'application/json');
        return this.http.post('http://funnybean.cloudapp.net/api/reservations/cars/byCar/' + id, JSON.stringify({ from: from, to: to }), { headers: headers });
    };
