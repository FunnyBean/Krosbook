...
export class ReservationService {
  ...
  public checkDupliciteRepeatingReservations(reservationId:number, repetition:string, interval:number, appearance:number)
  {
    let headers = new Headers();
    headers.append('Content-Type', 'application/json');
    return this.http.post('/api/reservations/rooms/checkForDuplicity/', JSON.stringify({ reservationId, repetition, interval, appearance }), {headers});
  }
}
