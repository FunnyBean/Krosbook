...
export class CarOrderService {
  ...
  public addOrder(carId, dateStart, dateEnd, destination, GPSSystem, privateUse, requirements, travelInsurance, reservationState) {
    let headers = new Headers();
    headers.append('Content-Type', 'application/json');
    return this.http.post('http://funnybean.cloudapp.net/api/reservations/cars', JSON.stringify({carId, dateStart, dateEnd, destination, GPSSystem, privateUse, requirements, travelInsurance, reservationState}), {headers});
  }
  ...
  public editOrder(id:number, carId, userId, dateStart, dateEnd, destination, GPSSystem, privateUse, requirements, travelInsurance, reservationState){
    let headers = new Headers();
    headers.append('Content-Type', 'application/json');
    return this.http.put("http://funnybean.cloudapp.net/api/reservations/cars/"+id, JSON.stringify({id, carId, userId, dateStart, dateEnd, destination, GPSSystem, privateUse, requirements, travelInsurance, reservationState}), {headers});
  }
  ...
  public getReservations(id:number, from:any, to:any){
    let headers = new Headers();
    headers.append('Content-Type', 'application/json');
    return this.http.post('http://funnybean.cloudapp.net/api/reservations/cars/byCar/'+id, JSON.stringify({from, to}), {headers});
  }
