...
export class CarOrderService {
  ...
  public getOrders() {
    ...
    return this.http.get('/api/reservations/cars/', { headers  } ); 
  }

  public getOrder(id:number) {
    let headers = new Headers();
    headers.append('Content-Type', 'application/json');
    return this.http.get('/api/reservations/cars/'+id, { headers  } ); 
  }
  ...
  public editOrder(id:number, carId, dateStart, dateEnd, destination, GPSSystem, privateUse, requirements, travelInsurance, reservationState){
    let headers = new Headers();
    headers.append('Content-Type', 'application/json');
    return this.http.put("/api/reservations/cars/"+id, JSON.stringify({id, carId, dateStart, dateEnd, destination, GPSSystem, privateUse, requirements, travelInsurance, reservationState}), {headers});
  }
